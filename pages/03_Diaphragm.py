...
class Rigid_Diaphragm():
    def __init__(self, L, H, CM_x, CM_y, dia_angle=0, w_ecc = 0.15, eq_ecc = 0.05):
        ...
        self.H = H # width of diaphragm NW dimension (ft)
        ...
        self.wind_pos = [self.CM, self.CM] # Wind Load application position @ center of diaphragm (x dir,y dir) (ft)
        self.wind_pos_ecc = [[[self.CM[0], self.CM[1] + self.H * self.w_ecc], # X direction plus eccentricity
                               [self.CM[0], self.CM[1] - self.H * self.w_ecc]], # X direction minus eccentricity
                             [[self.CM[0] - self.L * self.w_ecc, self.CM[1]], # Y direction plus eccentricity
                               [self.CM[0] - self.L * self.w_ecc, self.CM[1]]]] # Y direction minus eccentricity
        self.eq_pos = self.wind_pos # Seismic load applicaiton (same as wind)
        self.eq_pos_ecc = [[[0,0], [0,0]],[[0,0], [0,0]]] # Seismic Load eccentricity [X[pos(x,y), neg(x,y)], Y[pos(x,y), neg(x,y)]] (ft)
